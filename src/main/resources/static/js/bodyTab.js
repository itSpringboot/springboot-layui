...
layui.define(["element","jquery"],function(exports){
	var element = layui.element,
		...
		Tab = function(){
			this.tabConfig = {
				topMenu:undefined,//是否展现顶部菜单
				maxTopMenus:undefined,//顶部菜单展现最多数量
    ...
    Tab.prototype.navBar = function(strData){
        ...
        if(typeof(strData) == "string"){
            var data = JSON.parse(strData).menu.rows; //部分用户解析出来的是字符串，转换一下
        }else{
            data = strData.menu.rows;
        }
        var ulHtml = '';
        for(var i=0;i<data.length;i++){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                for(var j=0;j<data[i].children.length;j++){
					...
					if(data[i].children[j].icon==''||data[i].children[j].icon==null||data[i].children[j].icon.match(/\.(jpeg|jpg|gif|png)$/) != null){
						ulHtml += '<i class="layui-icon" data-icon="&#xe63c;">&#xe63c;</i>';
	...
	//生成左侧菜单
	Tab.prototype.navBarNoTop = function(strData){
		var data;
		if(typeof(strData) == "string"){
			var data = JSON.parse(strData).menu.rows; //部分用户解析出来的是字符串，转换一下
		}else{
		...
		var ulHtml = '';
		for(var i=0;i<data.length;i++){
			ulHtml +='<dd><a data-url="'+data[i].url+'">';
			var icon = data[i].icon;
			if(icon==''||icon==null||icon.match(/\.(jpeg|jpg|gif|png)$/) != null){
				ulHtml += '<i class="layui-icon" data-icon="&#xe63c;">&#xe63c;</i>';
			}else if(icon.indexOf("icon-") != -1){
			...
			if(data[i].children != undefined && data[i].children.length > 0){
				...
				for(var j=0;j<data[i].children.length;j++){
					ulHtml +='<dd><a data-url="'+data[i].children[j].url+'">';
					var icon = data[i].children[j].icon;
					if(icon==''||icon==null||icon.match(/\.(jpeg|jpg|gif|png)$/) != null){
						ulHtml += '<i class="layui-icon" data-icon="&#xe63c;">&#xe63c;</i>';
					}else if(icon.indexOf("icon-") != -1){
						ulHtml += '<i class="seraph '+icon+'" data-icon="'+icon+'"></i>';
					...
					ulHtml += '<cite>'+data[i].children[j].text+'</cite>';
					ulHtml += '</a>';
					ulHtml += '</dd>';
				}
				ulHtml += '</dl>';
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="page/main.html"><i class="layui-icon" data-icon=""></i><cite>首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-94);
	...
	//获取二级菜单数据
	Tab.prototype.renderNoTop = function(dl) {
		//显示左侧菜单
		var _this = this;
		dl.append(_this.navBarNoTop(dataStr));
		element.init();  //初始化页面元素
		$(window).resize(function(){
			$(".navBar").height($(window).height()-94);
		})
	...
	Tab.prototype.tabMove = function(){
		$(window).on("resize",function(event){
			var topTabsBox = $("#top_tabs_box"),
				...
				tabLiLeft = Math.ceil(tabLi.position().left),
				tabLiWidth = Math.ceil(tabLi.width())+30,
				top_tabs = document.getElementById("top_tabs"),
				topTabsBoxWidth = topTabsBoxWidth-220;
			...
			if(topTabsWidth > topTabsBoxWidth){
				if(tabLiLeft==0){
					...
					if(topTabsWidth == tabLiLeft+tabLiWidth){//最后一个时
						topTabs.css("left",-(topTabsWidth-topTabsBoxWidth-41));
					}else if(topTabsWidth-tabLiLeft<tabLiLeft&&tabLi.next().position().left+tabLi.next().width()>topTabsBoxWidth){
						topTabs.css("left",-(tabLi.next().position().left+tabLi.next().width()-topTabsBoxWidth-41+15));
					}else if(topTabsWidth-tabLiLeft>tabLiLeft&&topTabsWidth-tabLi.prev().position().left>topTabsBoxWidth){
						topTabs.css("left",-(tabLi.prev().position().left-41));
					}
				}
...
// 绑定左右移动切换
	$("body").on("click",".pressKey",function(){
		var topTabsBox = $("#top_tabs_box"),
			topTabsBoxWidth = $("#top_tabs_box").width(),
			topTabs = $("#top_tabs"),
			topTabsWidth = $("#top_tabs").width(),
